...
<html lang="en">
    ...
    <table style="width: 100%">
        ...
        <tr>
            <td style="text-align: right; padding: 5px">MAC của Slave:</td>
            <td><input id="tbxMACSlave"/></td>
        </tr>
    </table>
    <div style="text-align: right; padding-right: 10px">   
        <button class="NutNhan" id="btnMACSlave" onclick="LuuMACSlaveVaoEEPROM()">LƯU MACs</button>
    </div>  
    <div id="ThongBao"></div>
	</body>
<html>
...
//-----------------------------------------------------------------------------------------------------
async function LuuMACSlaveVaoEEPROM() {
    var MACSlave = document.getElementById("tbxMACSlave").value;
    xhttp.open("GET","/LuuMACSlaveVaoEEPROM?MACSlave=" + MACSlave, true);
    xhttp.onreadystatechange = KetQuaLuuMACs;//nhận reponse 
    xhttp.send();
}
//--------------------------------Kiểm tra response -------------------------------------------------//
function KetQuaLuuMACs(){
